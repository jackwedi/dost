...
class BirthdayModal extends React.Component {
    constructor(props) {
        super(props);
        this.state = { month: null, day: null, year: null, validDay: null, validYear: null, validForm: null, futureDOB: null };
    }
    ...
    checkInput() {
        const incorrectDay = isNaN(this.parsedDay) || !isFinite(this.state.day) || this.state.day.length > 2 || this.parsedDay > 31 || !this.parsedDay;
        const incorrectYear = isNaN(this.parsedYear) || !isFinite(this.state.year) || this.state.year.length !== 4;
        ...
        this.setState({validDay: !incorrectDay, validYear: !incorrectYear, validForm , futureDOB });
    }
